...
@FragmentScope
public class ProjectListPresenter extends BasePresenter<ProjectListContract.IProjectListModel, ProjectListContract.IProjectListView> {

    @Inject
    public ProjectListPresenter(ProjectListContract.IProjectListModel model, ProjectListContract.IProjectListView rootView) {
        super(model, rootView);
    }

    public void getProjectList(int page, int cid) {
        mModel.getProjectList(page, cid).subscribeOn(Schedulers.io())
                ...
                .subscribe(new Observer<BaseBean<BasePageBean<List<ArticleBean>>>>() {
                    ...
                    public void onNext(@NotNull BaseBean<BasePageBean<List<ArticleBean>>> bean) {
                        if (bean.getErrorCode() == Constant.SUCCESS) {
                            mRootView.showProjectList(bean.getData());
                        } else {
